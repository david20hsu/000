...
$title = "嘉南羊乳-宅配應收B2C(2聯)";
require_once("html/header.html");
require_once("html/menu.html");
//$comp_id="";
$ymx=date('Y-m') . '-%';
$ym = date('Y-m') . '-%';
$comp_idno=$comp_idnox="";
$xchk = 0;
...
$currentMonth = date('m'); foreach($monthArr as &$value); // 取得當月
$comp_idnox = '99776356';
if (isset($_POST['submit']) != "") {
    $comp_idnox = $_POST['comp_idnox'];
    $yy=$_POST['yy'];
  //  $month=$_POST['month'];
            ...
            <div class="form-group">
                <div class="row">
                    <nobr>
                        ...
                        公司:</label>
                        <select name="comp_idnox" id="comp_idnox" style="background-color:lightskyblue">
                            <?php
                            require('./include/config.php');
                            $sql = " select comp_idno,comp_names from comp where api_idno<>'' order by comp_idno";
                            $sth = $conn->prepare($sql);
                            $sth->execute();
                            echo "<option value=''>請點選</option>";
                            while ($row = $sth->fetch()) {
                                if ($row['comp_idno'] == $comp_idnox) {
...
                               <?php else : ?>   <!--echo $currentMonth-->                 <!--echo $currentMonth-->
...
                           <?php endforeach;?>
    ...
    <nobr>
        ...
        &nbsp;
        <button class="btn btn-primary btn-sm mt-1" data-toggle="modal" data-target="#QryModal">查詢</button>
        &nbsp;
...
<table id="myDataTalbe" class="table table-bordered table-striped table-hove" style="width:100%">
    ...
    <tbody>
        ...
        if (isset($_POST['Qrysubmit'])) {
            ...
            $sql = "select * from tax_b2c where tp='2' and datetime >='$sdate' and datetime <='$edate'";
            if ($comp_idno !== '') {
           ...
           // $edate = date('Y-m-d', strtotime("$sdate +4 month -1 day"));
            //$sql = "select * from tax_b2c where  uncode='$comp_idnox' and tp='2' and datetime between '$sdate' and '$edate' order by uncode,tax_id ";
            $sql = "select * from tax_b2c where  uncode='$comp_idnox' and tp='2' and datetime like '$ymx' order by uncode,tax_id ";
            unset($_POST);
        }
        //echo $sql;
        //exit();
        ...
        ?>
            <tr>
                ...
                <td><?php echo $row["narea_id"]; ?></td>
                <td>
                    <?php if ($row['tax_status'] == '0') { ?>
                        <?php if ($row["tax_id"] <> '') { ?>
                            <a onclick="return confirm('【已取號】確定要作廢嗎 ?')" href="tax/del_hrb2c.php?tax_id=<?php echo $row['tax_id'] . "&uncode=" .  $row['uncode']; ?>" class='btn-sm btn-danger mt-1'>作廢</a>
                        <?php } else { ?>
                            <a onclick="return confirm('【未取號】確定要刪除嗎 ?')" href="tax/del_b2c.php?orderCode=<?php echo $row['orderCode'] . "&tp=" .  $row['tp'] . "&uncode=" .  $row['uncode'] . "&xfile=../hrb2c_tax_list.php"; ?>" class='btn-sm btn-danger mt-1'>刪除</a>
                        <?php } ?>
                    <?php } else {
                        echo '已作廢';
                    } ?>
...
<?php require('tax/hrb2c_modal_add.php'); ?>
<!--Start QryModal-->
