...
if (isset($_GET)) {
    ...
    $sql="select a.*,b.api_sign,b.api_idno from tax_b2b a left join comp b on a.uncode=b.comp_idno where a.tp='2' and uncode='$uncode' and a.tax_id='$tax_id'";
  //  echo $sql;
  //  exit;
    ...
    if ($Rowcount>0){
        ...
        //$sing =strtoupper(md5((string)msectime().'F225672500'.'h564gh8dert')); 
        ...
        if ($obj->{'success'}=='true'){
            ...
            $cql = "UPDATE `tax_b2b` SET `tax_id`=:tax_id,`content`=:content,`tax_status`=:tax_status,`edate`=:edate where `tp`=:tp  and `orderCode`=:orderCode";

            $stmt = $conn->prepare($cql); 
            $stmt->execute($data);
            $msg="宅配應收B2B發票:".$tax_id.",作廢成功";  
        }else{
            $msg="宅配應收B2B發票:".$tax_id.",作廢失敗";  
        }
    }
