<?php
session_start();
if(!isset($_SESSION["username"])){
   header("Location: ../login.php");
   exit(); 
}
require_once("../html/header.html");
require_once("../html/menu.html");
$msg="<p align='center'><span style='font-size:30px;'>團戶應收B2C重傳(取號)..沒有發票號碼空白資料".'</span></p>';  
if (isset($_POST['submit'])) 
{
    $comp_idno=$_POST['comp_idno'];
    $yy=$_POST['yy'];
    $month=$_POST['month'];
    $ym=$yy.'-'.$month.'-%';
    if ($comp_idno!='' && $yy!='' && $month!='' )
    {
       $sql="select a.*,b.api_idno,api_sign from tax_b2c a left join comp b on a.uncode=b.comp_idno where a.datetime like '$ym' and a.tp='3' and a.tax_status='0' and  (a.tax_id='' or a.tax_id is null)";
       $msg= Rebud_json($sql);
     }else{
        $msg="請正確點選 公司及輸入，年度、月份";
    }
}
function post_json_data($url, $data_string) {
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_POST, 1);
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_POSTFIELDS, $data_string);
    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        'Content-Type: application/json; charset=utf-8',
        'Content-Length: ' . strlen($data_string))
    );
    ob_start();
    curl_exec($ch);
    $return_content = ob_get_contents();
    ob_end_clean();
    $return_code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
   // return array($return_code, $return_content);
    return $return_content;
  }
  
  function msectime()
  {
      list($msec, $sec) = explode(' ', microtime());
      $msectime = (float)sprintf('%.0f', (floatval($msec) + floatval($sec)) * 1000);
      return $msectime;
  }
  function Rebud_json($sql){
    require('../include/config.php'); 
   // $sql="select a.*,b.api_idno,api_sign from tax_b2c a left join comp b on a.uncode=b.comp_idno where a.datetime like '$ym' and a.tp='3' and a.tax_status='0' and  (a.tax_id='' or a.tax_id is null)";
    $i = 0;$j=0;
    $sth = $conn->prepare($sql);
    $sth->execute();
    while ($row= $sth->fetch()){
        $json_data=array();//create the array  
        $sing= strtoupper(md5((string)msectime().$row['api_idno'].$row['api_sign']));
        //$sing =strtoupper(md5((string)msectime().'H127633930'.'96kf28')); 
        if ($row['phone']==""){
            $customerName=$row['phone'];
            $phone=$row['customerName']; 
         }else{
            $customerName=$row['customerName'];
            $phone=$row['phone']; 
         }
        $json_data=[
           "timeStamp"=>(string)msectime(), //當前時間毫秒數（五分鐘內有效）
           "uncode"=>$row['uncode'],       // $row[0]; // 賣方統編 企業統一編碼
           "idno"=>$row['api_idno'],       // 登入身份證號
           "sign"=>$sing,
           "customerName"=>$customerName,    // 客戶代號 $row['customerName']
           "phone"=>$phone,
           "orderCode"=>$row['orderCode'],       // 預收單號
           "datetime"=>date('Y-m-d'),   //  發票日期
           "email"=>$row['email'],            // 客戶郵件
           "state"=>$row['state'],           //發票捐贈 無-0  捐贈-1*
           "donationCode" =>$row['donationCode'],   // 捐贈碼（當發票捐贈為1是必填）
           "totalFee"=>$row['totalFee'], //總價 , 預收金額
           "content"=>$row['content'],
           "items"=>[
              ["name"=>$row['name'],
              "money"=>$row['money'],
              "number"=>$row['number']
             ]]   
        ];
        $tax_id="";
        $data_string = json_encode($json_data,JSON_UNESCAPED_UNICODE);
        $json= post_json_data('https://www.giveme.com.tw/invoice.do?action=addB2C', $data_string);
        $obj = json_decode($json);
        if ($obj->{'success'}=='true'){
            $tax_id= $obj->{'code'};
            $i+=1;
        }else{
         $j+=1;
        }
        $data = [
            'tax_id' => $tax_id,
            'tax_date' =>date('Y-m-d'),
            'tp' =>'3',
            "orderCode"=>$row['orderCode'],
            "totalFee"=>$row['totalFee']
        ]; 
        $cql = "UPDATE `tax_b2c` SET `tax_id`=:tax_id,`datetime`=:tax_date where `tp`=:tp  and `orderCode`=:orderCode and `totalFee`=:totalFee";
        $stmt = $conn->prepare($cql); 
        $stmt->execute($data); 
    }
    $msg="團戶應收B2C重傳(取號): ".$i.' 筆, 失敗: ' .$j.' 筆</span></p>'; 
    return $msg;
 }
  ?>
  <p></p>
<div class='err'>
<p align='center'><span style='font-size:20px;'>
<?php
        if($msg<>''){
         echo $msg;
        }
?>
</span></p>
</div>
<p></p>
  <p align='center'><a href="../grb2c_tax_list.php"  class="btn btn-info" role="button">返回</a> </p>
  <?php
  require("../html/footer.html");
  ?>
  </body>
  </html>