...
if (isset($_GET)) {
    ...
    $sql="select a.*,b.api_sign,b.api_idno from tax_b2c a left join comp b on a.uncode=b.comp_idno where a.tp='1' and uncode='$uncode' and a.tax_id='$tax_id'";
      ...
      //  print_r($obj);
       // exit();
        if ($obj->{'success'}=='true'){
            ...
            $cql = "UPDATE `tax_b2c` SET `tax_id`=:tax_id,`content`=:content,`tax_status`=:tax_status,`edate`=:edate where `tp`=:tp  and `orderCode`=:orderCode";
            $stmt = $conn->prepare($cql); 
            $stmt->execute($data);
            $msg="<p align='center'><span style='font-size:25px;'>宅配預收B2C發票:".$tax_id.',作廢成功</span></p>';  
        }else{
            $msg="<p align='center'><span style='font-size:25px;color:Blue;'>宅配預收B2C發票:".$tax_id.',作廢失敗</span></p>';  
        }
    }
