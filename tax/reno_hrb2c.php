...
if (isset($_POST['submit'])) {
  ...
  if ($comp_idno != '' && $yy != '' && $month != '') {
    $sql = "select a.*,b.api_idno,api_sign from tax_b2c a left join comp b on a.uncode=b.comp_idno where a.uncode='$comp_idno' and  a.datetime like '$ym' and a.tp='2' and a.tax_status='0' and  (a.tax_id='' or a.tax_id is null) limit $listno ";
    //echo $sql;
    //exit();
    $msg = Rebud_json($sql);
  } else {
...
function Rebud_json($sql)
{
  ...
  while ($row = $sth->fetch()) {
    ...
    $json_data = [
      ...
      "phone" => $phone,                  //載具 $row['phone']
      "orderCode" => $orderCode,          // 單號-YYYmm
