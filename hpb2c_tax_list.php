...
<div class="row" id="toprow"> <!--class="row justify-content-center-->
<div class="col-9">
...
<thead>
   ...
   //    require('include/config.php'); 
      if (isset($_POST['Qrysubmit'])){
          ...
          $sql="select * from tax_b2c where tp='1' and datetime >='$sdate' and datetime <='$edate'";
          if ($comp_idno!==''){
          ...
          // $sdate =$xy.'-'.$xm.'-01';
           $edate =date('Y-m-d', strtotime("$sdate +4 month -1 day"));
            $sql="select * from tax_b2c where tp='1' and  datetime between '$sdate' and '$edate' order by datetime desc ";
        }
      //  echo $sql;
        ...
        <tr>
            ...
            <td style="text-align:right;"><?php echo  number_format($row["money"],0);?> </td>
            <td style="text-align:right;"><?php echo  number_format($row["number"]); ?></td>    
            <td><?php echo $row["content"]; ?></td> 
            <td><?php echo $row["narea_id"]; ?></td>     
            <td>
            <?php if ($row['tax_status']=='0') { ?>
                  <?php if ( $row["tax_id"]<>''){?>
                    <a onclick="return confirm('【已取號】確定要作廢嗎 ?')" href="tax/del_hpb2c.php?tax_id=<?php echo $row['tax_id'] . "&uncode=" .$row['uncode']; ?>" class='btn-sm btn-danger mt-1'>作廢</a>
                  <?php }else{ ?>
                    <a onclick="return confirm('【未取號】確定要刪除嗎 ?')" href="tax/del_b2c.php?orderCode=<?php echo $row['orderCode'] . "&tp=" .$row['tp']."&uncode=" .  $row['uncode']."&xfile=../hpb2c_tax_list.php" ; ?>" class='btn-sm btn-danger mt-1'>刪除</a>
                   <?php }?>
              <?php }else{ echo '已作廢'; } ?>
          </td>
        </tr>
...
<?php require('tax/hpb2c_modal_add.php');?>
<!--Start QryModal-->
    ...
    $(function() {
       ...
       dom: 'Blfrtip',
        ...
        render: function(data, type, row, meta) {
            ...
            //代表，是顯示型別的時候判斷值的長度是否超過8，如果是則擷取
            //這裡只處理了型別是顯示的，過濾和排序返回原始資料
            if (type === 'display') {
                if (data.length > 8) {
