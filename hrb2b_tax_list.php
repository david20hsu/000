...
$currentMonth = date('m');
foreach ($monthArr as &$value); // 取得當月
$comp_idnox = '99776356';
...
<table id="myDataTalbe" class="table table-bordered table-striped table-hove" style="width:100%">
    ...
    <tbody>
        ...
        if (isset($_POST['Qrysubmit'])) {
            ...
            $sql = "select *  from tax_b2b where tp='2' and  datetime >='$sdate' and datetime <='$edate'";
            if ($comp_idno !== '') {
            ...
            $sql = "select *  from tax_b2b where uncode='$comp_idnox' and tp='2' and datetime like '$ymx' order by  uncode,tax_id";
            ...
            <tr>
                ...
                <td>
                    <?php if ($row['tax_status'] == '0') { ?>
                        <?php if ($row["tax_id"] <> '') { ?>
                            <a onclick="return confirm('【已取號】確定要作廢嗎 ?')" href="tax/del_hrb2b.php?tax_id=<?php echo $row['tax_id'] . "&uncode=" .  $row['uncode']; ?>" class='btn-sm btn-danger mt-1'>作廢</a>
                        <?php } else { ?>
                            <a onclick="return confirm('【未取號】確定要刪除嗎 ?')" href="tax/del_b2b.php?orderCode=<?php echo $row['orderCode'] . "&tp=" .  $row['tp'] . "&uncode=" .  $row['uncode'] . "&xfile=../hrb2b_tax_list.php"; ?>" class='btn-sm btn-danger mt-1'>刪除</a>
                        <?php } ?>
                    <?php } else {
                        echo '已作廢';
                    } ?>
...
<?php require('tax/hrb2b_modal_add.php'); ?>
<?php require('tax/hrb2b_modal_edit.php'); ?>
<!--Start QryModal-->
