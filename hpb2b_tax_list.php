    ...
    <tbody>
        ...
        if (isset($_POST['Qrysubmit'])) {
           ...
           // $sdate = $xy . '-' . $xm . '-01';
            ...
            $sql = "select *  from tax_b2b where tp='1' and datetime  between '$sdate' and '$edate' order by  uncode,tax_id";
        }
        ...
        ?>
            <tr>
                ...
                <td style="text-align: right"><?php echo number_format($row["sales"]); ?></td>
                <td><?php echo $row["name"]; ?></td>
                <td style="text-align: right"><?php echo number_format($row["money"]); ?> </td>
                <td style="text-align: right"><?php echo number_format($row["number"]); ?></td>
                <td><?php echo $row["content"]; ?></td>
                <td>
                    <?php if ($row['tax_status'] == '0') { ?>
                        <?php if ($row["tax_id"] <> '') { ?>
                            <a onclick="return confirm('【已取號】確定要作廢嗎 ?')" href="tax/del_hpb2b.php?tax_id=<?php echo $row['tax_id'] . "&uncode=" .  $row['uncode']; ?>" class='btn-sm btn-danger mt-1'>作廢</a>
                        <?php } else { ?>
                            <a onclick="return confirm('【未取號】確定要刪除嗎 ?')" href="tax/del_b2b.php?orderCode=<?php echo $row['orderCode'] . "&tp=" .  $row['tp'] . "&uncode=" .  $row['uncode'] . "&xfile=../hpb2b_tax_list.php"; ?>" class='btn-sm btn-danger mt-1'>刪除</a>
                        <?php } ?>
                    <?php } else {
                        echo '已作廢';
                    } ?>
...
<?php require('tax/hpb2b_modal_add.php'); ?>
<?php require('tax/hpb2b_modal_edit.php'); ?>
<!--Start QryModal-->
